...
def test_extract_upload_file_context_reads_text_file() -> None:
    upload_file = UploadFile(
        filename="notes.txt",
        file=BytesIO("第一行\n第二行".encode("utf-8")),
    )

    file_context = asyncio.run(extract_upload_file_context(upload_file))

    assert file_context.filename == "notes.txt"
    assert "第一行" in file_context.content
