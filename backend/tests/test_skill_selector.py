...
from doc_process_studio.services.skill import selector as selector_module
from doc_process_studio.services.skill.selector import (
    SelectorOption,
...
from doc_process_studio.settings import settings


def _build_skill(skill_id: str) -> SkillInterfaceConfig:
    return SkillInterfaceConfig(
        id=skill_id,
        display_name=skill_id,
        short_description=skill_id,
...
def test_select_skills_with_planner_uses_defaults_and_normalization(monkeypatch) -> None:
    ...
    decision = asyncio.run(
        select_skills_with_planner(
            ...
            available_skills=[_build_skill("document-assistant")],
...
def test_select_for_chat_skills_uses_chat_template_defaults(monkeypatch) -> None:
    captured: dict[str, object] = {}
...
def test_select_for_workspace_reference_uses_reference_template_defaults(monkeypatch) -> None:
    captured: dict[str, object] = {}
    ...
    decision = asyncio.run(
        select_for_workspace_reference(
            ...
            available_skills=[
                _build_skill("body-sections/common.md"),
                _build_skill("body-sections/impact.md"),
                _build_skill("body-sections/timeline.md"),
            ],
