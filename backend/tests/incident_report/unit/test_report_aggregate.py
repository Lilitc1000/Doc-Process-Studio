...
class TestReportCreate:
    ...
    def test_create_produces_created_event(self):
        report = Report.create(
            report_id="rep-1",
            ref_no="DAS-0001",
            title="测试",
...
class TestReportSubmit:
    ...
    def test_submit_produces_submitted_event(self):
        report = _make_report(status=ReportStatus.DRAFT, form_data=_full_form_data())
...
class TestReportApprove:
    ...
    def test_approve_produces_approved_event(self):
        report = _make_report(status=ReportStatus.PENDING)
...
class TestReportAssignHandler:
    def test_assign_from_approved_advances_to_in_progress(self):
    ...
    def test_assign_from_in_progress_keeps_status(self):
    ...
    def test_assign_from_draft_raises(self):
        report = _make_report(status=ReportStatus.DRAFT)
        with pytest.raises(InvalidTransitionError):
            report.assign_handler(
                actor_id="usr_verifier",
...
class TestReportClose:
    def test_close_from_in_progress_to_closed(self):
    ...
    def test_close_produces_closed_event(self):
        report = _make_report(status=ReportStatus.IN_PROGRESS)
...
class TestReportReopen:
    def test_reopen_from_closed_to_draft(self):
        report = _make_report(
    ...
    def test_reopen_produces_reopened_event(self):
