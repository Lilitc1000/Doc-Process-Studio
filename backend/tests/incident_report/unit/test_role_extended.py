...
def test_assign_incident_role_existing(mock_session):
    from doc_process_studio.incident_report.models.incident_report_role import IncidentReportRole

    existing = IncidentReportRole(user_id="usr_test", role="reporter", assigned_by="usr_admin")
    mock_scalar = MagicMock()
    mock_scalar.scalar_one_or_none.return_value = existing
    mock_session.execute.return_value = mock_scalar
...
def test_list_all_role_assignments(mock_session):
    ...
    role1 = IncidentReportRole(user_id="usr_1", role="reporter", assigned_by="usr_admin")
    role2 = IncidentReportRole(user_id="usr_2", role="admin", assigned_by="usr_admin")
    mock_result = MagicMock()
