...
def test_extract_reference_summary_basic() -> None:
    text = "# Title\nLine 1\nLine 2\nLine 3\nLine 4"
...
def test_extract_reference_summary_no_headings() -> None:
    text = "Line 1\nLine 2\nLine 3"
    result = _extract_reference_summary(text)
    assert "Line 1" in result


def test_extract_reference_summary_empty() -> None:
    result = _extract_reference_summary("")
    assert result == ""


def test_select_references_by_heuristic_common() -> None:
...
def test_select_references_by_heuristic_no_match() -> None:
...
def test_select_references_by_heuristic_empty_paths() -> None:
    result = _select_references_by_heuristic(section_id="impact", available_paths=[])
    assert result == []


def test_select_references_by_heuristic_quick() -> None:
...
def test_load_text_file_nonexistent() -> None:
