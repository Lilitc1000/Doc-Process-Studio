...
def test_quick_generate_body_updates_form_answers_and_trace(monkeypatch) -> None:
    ...
    async def fake_touch_incident_session_updated_at(session_id: str) -> None:
        assert session_id == "incident-session-1"
        assert score > 0
    ...
    monkeypatch.setattr(
        ...
        fake_touch_incident_session_updated_at,
    )
    monkeypatch.setattr(
        generation_module,
...
def test_quick_generate_body_respects_english_input_language(monkeypatch) -> None:
    ...
    async def fake_touch_incident_session_updated_at(_session_id: str) -> None:
        return None
