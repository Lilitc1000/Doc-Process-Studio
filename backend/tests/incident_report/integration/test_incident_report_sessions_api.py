...
def test_api_incident_report_sessions_list(monkeypatch, auth_headers) -> None:
    ...
    response = client.get("/api/incident-report/sessions", headers=auth_headers)

    assert response.status_code == 200
    assert response.json()["sessions"][0]["id"] == "incident-session-1"


def test_api_incident_report_session_update(monkeypatch, auth_headers) -> None:
...
def test_api_incident_report_preview(monkeypatch, auth_headers) -> None:
