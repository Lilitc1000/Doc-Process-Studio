...
def test_get_read_context_relative_path_empty_chunks():
    tool_result = {"chunks": []}
    result = _get_read_context_relative_path(tool_result)
    assert result is None


def test_get_read_context_relative_path_no_chunks():
    tool_result = {}
    result = _get_read_context_relative_path(tool_result)
    assert result is None


def test_get_read_context_relative_path_non_string_source():
    tool_result = {"chunks": [{"source_path": 123}]}
    result = _get_read_context_relative_path(tool_result)
    assert result is None


def test_get_read_context_relative_path_empty_source():
