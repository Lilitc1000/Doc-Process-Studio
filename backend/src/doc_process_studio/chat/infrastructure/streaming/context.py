...
from ....skill.application.dtos import SkillInterfaceConfig
...
def _build_skills_catalog_lines(active_skill_ids: list[str]) -> list[str]:
    interfaces = {skill.id: skill for skill in list_skill_interfaces() if skill.id in set(active_skill_ids)}
    lines: list[str] = []
    for skill_id in active_skill_ids:
        ...
        if skill is None:
            continue
        if is_kb_skill_id(skill_id):
            lines.append(f"- {skill.id}: {skill.short_description or '无描述'} (知识库 RAG)")
        else:
            skill_path = (SKILLS_DIR / skill_id / "SKILL.md").as_posix()
            lines.append(f"- {skill.id}: {skill.short_description or '无描述'} (file: {skill_path})")
    return lines
...
def build_multi_skill_runtime_instructions(
    *,
    ...
    explicit_line = (
        "本轮用户显式选择了以下文档处理方式，必须优先使用：" + ", ".join(explicit_skill_ids)
        if explicit_skill_ids
        else "本轮用户未显式选择文档处理方式，你可以根据任务描述从可用方式中选择最小集合隐式调用。"
    )
    implicit_line = (
        "规划层根据本轮用户输入追加了以下隐式文档处理方式：" + ", ".join(implicit_skill_ids or [])
    ...
    return "\n".join(
        [
            ...
            *catalog_lines,
            ...
            "触发规则：若用户通过输入框显式选择，或在消息中使用 $SkillName 明确提及某方式，"
            "当前轮必须使用；若提及多个，必须全部使用；除非再次提及，不跨轮沿用。",
            "未显式选择时，可根据任务描述按最小集合隐式调用；隐式调用由规划层建议，可按实际任务取舍。",
            "技能使用方法（渐进式披露）：先读 SKILL.md 必要部分；若引用 references/，"
            "仅按需读取必要文件；优先复用 scripts/ 与 assets/。",
...
def format_output_name_from_template(
    ...
    format_payload = {key: value for key, value in payload.items() if isinstance(value, (str, int, float))}
    try:
...
def merge_uploaded_files_context(
    *contexts: str | None,
    ...
    normalized_sections = [context.strip() for context in contexts if isinstance(context, str) and context.strip()]
