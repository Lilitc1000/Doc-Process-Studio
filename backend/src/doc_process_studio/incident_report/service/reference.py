...
def load_incident_skill_markdown() -> str:
...
def build_incident_reference_catalog() -> list[dict[str, str]]:
...
async def resolve_generation_reference_context(
    *,
    ...
    reference_catalog = build_incident_reference_catalog()
    ...
    try:
        planner_query = _build_reference_planner_query(
            ...
            skill_markdown=load_incident_skill_markdown(),
        )
    ...
    for relative_path in selected_files:
        text = load_text_file(INCIDENT_REPORT_REFERENCE_DIR / relative_path)
        if not text:
            continue
