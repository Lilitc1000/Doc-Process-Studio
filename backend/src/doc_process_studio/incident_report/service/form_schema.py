...
class FormFieldSchema(BaseModel):
    ...
    field_type: str = Field(..., description="字段类型: text/textarea/select/date/datetime/rich_text/timeline/attachment")
    ...
    steps=[
        FormStepSchema(
            ...
            fields=[
                ...
                FormFieldSchema(field_id="manual_system", label="系统", field_type="text", required=True, placeholder="如：数据库系统"),
                FormFieldSchema(field_id="manual_location", label="位置", field_type="text", placeholder="如：机房A"),
                FormFieldSchema(field_id="manual_fault_symptom", label="故障现象", field_type="textarea", required=True),
                FormFieldSchema(field_id="manual_status", label="状态", field_type="select", options=[
                    {"value": "open", "label": "开放"},
        ...
        FormStepSchema(
            ...
            fields=[
                FormFieldSchema(field_id="quick_narrative", label="事故简述（快填）", field_type="textarea", placeholder="简要描述事故，AI将自动生成详细报告"),
