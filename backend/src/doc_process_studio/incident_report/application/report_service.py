...
class ReportApplicationService:
    ...
    async def update(self, cmd: UpdateReportCommand) -> IncidentReportDetail | None:
        ...
        report.update_fields(actor_id=cmd.actor_id, **cmd.fields)
    ...
    async def assign_handler(self, cmd: AssignHandlerCommand) -> IncidentReportDetail | None:
        ...
        # 解析 assignee 用户名用于审计 comment
        assignee_names = await self._users.resolve_usernames({cmd.assignee_id})
        assignee_display = assignee_names.get(cmd.assignee_id, cmd.assignee_id)
