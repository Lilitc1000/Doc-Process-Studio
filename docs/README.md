...
| 变更文件 | 执行命令 | 检查内容 |
|---------|---------|---------|
| `backend/{src,tests}/**/*.py` | `ruff check --ignore-noqa --select F,ARG src/ tests/` | 未使用 import（F401）、未使用变量（F841）、未使用函数参数（ARG） |
| `backend/{src,tests}/**/*.py` | `vulture src/` | 死代码检测 |
| `backend/{src,tests}/**/*.py` | `mypy src/doc_process_studio` | 静态类型检查 |
